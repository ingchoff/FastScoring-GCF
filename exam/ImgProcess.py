...
def choice_to_bubble(choice):
    column = (3 if choice > 75 else
              ...
              1 if choice > 25 else
              0)
    return (choice - (25*column+1)) * 20 + (5*column+1)


# check to index of bubble choice each questions
def calulate_score(questions_no, answersheet, subject_img, answer_keys):
    first_bubble = choice_to_bubble(questions_no)
...
def mask_choices_bubbled(choice_contours, bound_img):
    ...
    for (q, i) in enumerate(np.arange(0, len(questnts), 20)):
        # sort the contours for the current question from
        # left to right, then initialize the index of the
        # bubbled answer
        cnts = contours.sort_contours(questnts[i:i + 20])[0]
        bubbled = None
...
def main_process(form_img, subject_img, form_std_img, std_img):
    ...
    questionCnts = detect_circle(gray, 500)
